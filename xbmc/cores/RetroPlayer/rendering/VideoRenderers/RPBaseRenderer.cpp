...
void CRPBaseRenderer::ManageRenderArea(const IRenderBuffer& renderBuffer)
{
  ...
  const float frameAspectRatio =
      (renderBuffer.GetDisplayAspectRatio() > 0.0f) ? renderBuffer.GetDisplayAspectRatio()
                                                    : sourceFrameRatio;
  ...
  CRect destRect;
  CRenderUtils::CalcNormalRenderRect(viewRect, frameAspectRatio * pixelRatio, zoomAmount,
                                     destRect);
