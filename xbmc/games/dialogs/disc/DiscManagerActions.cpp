...
void CDiscManagerActions::OnSelectDisc()
{
  ...
  m_discManager.SelectDiscToInsert(
      ...
      [this, selectedIndex](std::optional<size_t> discIndex)
      {
        ...
        // Returning from an explicit selection of another disc should land
        // on Resume, otherwise stay on Select to allow for another attempt
        bool discChanged = false;
        if (selectedIndex.has_value() && discIndex.has_value())
          discChanged = (*selectedIndex != *discIndex);
        else
          discChanged = (selectedIndex.has_value() != discIndex.has_value());

        if (success && discChanged)
