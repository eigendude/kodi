...
std::string CDialogGameDiscChanger::GetHeader()
{
  ...
  auto discManager =
      windowManager.GetWindow<CDialogGameDiscManager>(WINDOW_DIALOG_GAME_DISC_MANAGER);
  if (discManager)
  {
    const CGameClientDiscModel& discModel = discManager->GetDiscModel();

    if (discModel.IsEjected())
    {
      header = CServiceBroker::GetResourcesComponent().GetLocalizeStrings().Get(162); // "Tray open"
    }
    else
    {
      const std::optional<size_t> selectedIndex = discModel.GetSelectedDiscIndex();
      if (selectedIndex.has_value())
        header = discModel.GetLabelByIndex(*selectedIndex);
      else
        header =
            CServiceBroker::GetResourcesComponent().GetLocalizeStrings().Get(35274); // "No disc"
    }
  }
