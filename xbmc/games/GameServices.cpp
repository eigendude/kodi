...
public:
  ...
  void Run(const CRYPTO::KeyPair& keyPair) override
  {
    ...
    KeyPair _keypair{PublicKey{{Key::Type::Ed25519,
    ...
    ServerContext server =
        insecure_mode ? initInsecureServer(_keypair) : initSecureServer(_keypair);
    ...
    server.io_context->post(
      [host{std::move(server.host)}]
      {
        ...
        CLog::Log(LOGDEBUG, "Server started. Listening on: {}, Peer ID: {}",
            ma.getStringAddress(),
            host->getPeerInfo().id.toBase58());
        CLog::Log(LOGDEBUG, "Connection string: {}/ipfs/{}",
            ma.getStringAddress(),
            host->getPeerInfo().id.toBase58());
      }
    );
...
void RunClient()
{
  ...
  //! @todo
  std::string multiaddr = LOCALHOST_MULTIADDRESS + "/12D3KooWEgUjBV5FJAuBSoNMRYFRHjV7PjZwRQ7b43EKX9g7D6xV";

  // Create IO context, which allows us to execute async operations
  std::shared_ptr<boost::asio::io_context> context = injector.create<std::shared_ptr<boost::asio::io_context>>();
  context->post(
    [host{std::move(host)}, &echo, &multiaddr]
    {
      ...
      host->newStream(
        peer_info,
        echo.getProtocolId(),
        [&echo](auto &&stream_res)
        {
          if (!stream_res)
          {
            CLog::Log(LOGERROR, "Cannot connect to server: {}",
          ...
          echo_client->sendAnd(
            ...
            [stream = std::move(stream_p)](auto &&response_result)
            {
              ...
              stream->close(
                [](auto &&)
                {
                  CLog::Log(LOGDEBUG, "Stream closed");
                }
              );
            }
          );
