...
{
protected:
  //! Root location where RetroPlayer stores per-game disc-state data.
  static std::string GetDiscStateDirectory();

  //! Sanitized, extensionless base file name derived from the provided game path.
  static std::string GetSafeBaseName(const std::string& gamePath);

  //! Per-game subdirectory name used to isolate state files by source path and CRC.
  static std::string GetStateSubdirectory(const std::string& gamePath);

  //! Build a complete state file path for the given extension (without leading dot).
