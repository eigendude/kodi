...
bool CGameClientDiscModel::AddDisc(const std::string& path, const std::string& cachedLabel)
{
  ...
  m_discs.push_back({GameClientDiscEntry::DiscSlotType::Disc, path, DeriveBasename(path), cachedLabel});
...
bool CGameClientDiscModel::RemoveDiscByIndex(size_t index)
{
  ...
  const bool wasSelectedDisc =
      (m_selectedType == DiscSelectionType::Disc && m_selectedDiscIndex.has_value() &&
...
{
  const auto it =
      std::find_if(m_discs.begin(), m_discs.end(), [&path](const GameClientDiscEntry& disc)
                   {
