...
{
  // Include a CRC of the full game path so identical filenames in different locations do not clash.
...
{
  const std::string safeBaseName = GetSafeBaseName(gamePath);
  const std::string safeFileName = StringUtils::Format("{}.{}", safeBaseName, extension);
